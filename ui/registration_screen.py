...
from .models import RegistrationState, ScreenEvent


class RegistrationScreen(BaseScreen):
    def __init__(self, stdscr: "curses.window") -> None:
        super().__init__(stdscr)
        self._state: RegistrationState = RegistrationState(
            screen_id="Register",
    ...
    def set_state(self, state: RegistrationState) -> None:
    ...
    def render(self) -> None:
        ...
        # Title
        self._draw_centered(top_y, "Register", curses.A_BOLD)
        ...
        # Error
        if s.error:
            self._draw_centered(field_top + len(s.fields) * 3 + 1,
                                s.error, curses.color_pair(3) | curses.A_BOLD)

        self._draw_centered(max_y - 2, s.help_text, curses.A_DIM)
        self.stdscr.refresh()

    def _make_state(self, s: RegistrationState, active_field: int = None, values: dict = None, error: str = None) -> RegistrationState:
    ...
    def handle_key(self, key: int) -> Optional[object]:
        ...
        if key == 27:                                   # Esc → go back
            return ScreenEvent(name="register.back", payload={})

        if key in (curses.KEY_DOWN, ord("\t")):
            self._state = self._make_state(s, active_field=(s.active_field + 1) % len(s.fields))
            return None
    ...
    def _submit(self) -> Optional[ScreenEvent]:
        ...
        confirm  = s.values.get("confirm_password", "")

        if not username:
            self._state = self._make_state(s, error="Username cannot be empty.")
            return None
        if len(password) < 6:
            self._state = self._make_state(s, error="Password must be at least 6 characters.")
            return None
        if password != confirm:
            self._state = self._make_state(s, error="Passwords do not match.")
            return None
